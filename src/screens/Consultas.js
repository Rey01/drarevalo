...
class Consultas extends React.Component {
  ...
  renderCard = (props, index) => {
    ...
    return (
      <Block row center card shadow space="between" style={styles.card} key={props.idConsulta}>
        <Gradient
          start={[0.45, 0.45]}
          end={[0.90, 0.90]}
        ...
        <Block flex>
          <Text size={BASE_SIZE * 1.125}>{props.fechaRegistro}</Text>
          <Text size={BASE_SIZE * 0.875} muted>{props.nombre}</Text>
        </Block>
